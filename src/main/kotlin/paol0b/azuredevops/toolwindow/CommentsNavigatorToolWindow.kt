...
class CommentsNavigatorPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    private val showCommentsToggle: JToggleButton
    ...
    private fun navigateToComment(item: CommentItem) {
        // Only navigate to comment if toggle is enabled
        if (!showCommentsToggle.isSelected) {
        ...
        openFiles.forEach { file ->
            ...
            editors.forEach { fileEditor ->
                if (fileEditor is com.intellij.openapi.fileEditor.TextEditor) {
                    commentsService.loadCommentsInEditor(fileEditor.editor, file, currentPullRequest!!)
                }
            }
        }
    }
